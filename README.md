...
這是一個專為歌手**李詩民**設計的沉浸式音樂故事專輯網頁。結合了**復古黑膠唱片機的實體互動**與**日記本故事手稿**的雙欄排版，20 首關於生命哲學、自我救贖的歌首尾相連，帶給聽眾都市療癒體驗。
...
├── albumData.js        <- 🗃️ 20 首歌曲的歌詞、Suno Prompt 與故事背景資料庫
...
2. **情感流動微光背景**：當切換 20 首不同主題的歌曲時，網頁背景大片磨砂毛玻璃微光會緩緩流動變色（如清晨是暖陽粉、雨天是憂鬱藍灰色、深夜是靛藍色），與李詩民的歌聲情緒完美契合。
...
為確保已經生成並上傳的真實 MP3 音檔不會在未來開發、重構、或生成佔位檔時遭到意外破壞或覆蓋，所有開發者與協同 AI 必須遵守以下安全守則：

1.  **音檔生成保護**：
    *   `generate_audio.js` 腳本已經內建了防覆蓋保護。在生成測試靜音音檔時，會先檢查 `assets/audio/trackXX.mp3` 是否已存在。
    *   **嚴禁**修改該腳本為強制覆蓋模式。
2.  **新歌追加守則**：
    *   如需追加新歌，僅需在 `generate_audio.js` 中將上限數字改大，執行腳本時它會自動**僅生成新增歌曲的佔位檔**，絕對不會影響原有音檔。
3.  **Git 版本防禦**：
    *   專案已使用 Git 進行完整版控。如不慎因腳本出錯導致音檔被修改或覆蓋，請立即在終端機中執行：
        ```bash
        git restore assets/audio/
        ```
        即可完全還原所有被覆蓋的音檔。
